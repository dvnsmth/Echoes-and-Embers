...
import { startingStats } from './stats.js'; // new model source of base 12s
...
export const UI = {
  ...
  // ========== Character Creator (12 pts, 10–60 band, STR/DEX/CON/INT/WIS/LCK) ==========
  resetAllocator() {
  ...
  updateAlloc() {
    ...
    const pts = document.getElementById('cc-points'); if (pts) pts.textContent = UI.alloc.points;
  },
  ...
  refreshParty() {
      ...
      card.innerHTML = `
        <div class="row" style="gap:8px;align-items:center">
          ...
          <div class="col" style="gap:2px">
            ...
            <span class="tag">${ch.race} ${ch.clazz} • Lv ${ch.level} • HP ${ch.hp}/${ch.maxHP} • Unspent ${ch.unspent}</span>
    ...
    if (pc) pc.textContent = `${State.party.length} / 4`;
  },

  xpPct(ch) {
    const cur = ch.xp, L = ch.level, a = DB.xpCurve[L], b = DB.xpCurve[L + 1] || a + 1;
    return Math.floor(((cur - a) / (b - a)) * 100);
  },
  ...
  // ========== Sheets panel (derived stats, not calcMod/meta.armor) ==========
  refreshSheet() {
    ...
    State.party.forEach(ch => {
      ...
      c.innerHTML = `
        <div class="row" style="gap:8px">
          ...
          <div class="col" style="gap:2px">
            <strong>${ch.name}</strong>
            <span class="tag">${ch.race} ${ch.clazz} • Lv ${ch.level} • XP ${ch.xp} • Unspent ${ch.unspent}</span>
          </div>
          <div class="spacer"></div>
        ...
        <div class="grid cols-3" style="margin-top:6px">
          ${['STR','DEX','CON','INT','WIS','LCK'].map(k => `
            <div class="stat"><b>${k}</b><span>${ch.stats[k]}</span></div>
          `).join('')}
        </div>
...
export const SettingsUI = {
  init() {
    ...
    // placeholders for your three options inside Settings
